...
const getWheelTargetLabels = createLocalizedCatalog<Record<SessionGridWheelTarget, string>>(() => ({
  focus: translate('sessionGrid.wheel.focus', 'According to focus'),
...
function WheelTargetHint({ target }: { target: SessionGridWheelTarget }): React.JSX.Element {
  ...
  if (target === 'focus') {
    return (
      <span className="inline-block max-w-52 whitespace-normal">
        {translate(
          'sessionGrid.wheel.focusHint',
          'Scrolls the focused terminal; otherwise the grid. {{shift}} + wheel reverses the target.',
          { shift }
        )}
      </span>
    )
    ...
    >
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          {/* pt-px: same optical nudge onto the ± icons as the page indicator. */}
          <button
            type="button"
            ...
            className="min-w-11 border-x border-border/60 px-1.5 pt-px text-[11px] tabular-nums text-muted-foreground hover:bg-accent hover:text-foreground"
          >
            {formatZoom(zoom)}
          </button>
        </TooltipTrigger>
