...
function mount(mode: SessionGridScrollMode, wheelTarget: SessionGridWheelTarget = 'focus') {
...
describe('useSessionGridScroll', () => {
  ...
  describe('wheel', () => {
    ...
    it('delivers the wheel to xterm before containment, unless Shift forces the grid', () => {
      const { container, result } = mount('row', 'terminal')
      const xterm = addTerminal(container)

      const delivered = vi.fn()
      xterm.addEventListener('wheel', delivered)
    ...
    it('keeps a grid gesture over a terminal sliding under the stationary pointer', () => {
      const { container, result } = mount('row', 'terminal')
      const xterm = addTerminal(container)
      wheel(container, 40)
      expect(result.current.currentPosition).toBe(1)
    ...
    it('leaves every plain wheel over a terminal to the card under the terminal target', () => {
      ...
      expect(wheel(xterm, 40).defaultPrevented).toBe(true)
      expect(result.current.currentPosition).toBe(0)
