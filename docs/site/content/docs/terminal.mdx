...
Previewable HTML file links can open in Orca from local, SSH, and paired-runtime workspaces; remote file rows also offer **Download & open with default app**. Local file links open in the workspace where you clicked them, even if the file is open in another workspace. Remote file links use the owning workspace when its host requires that route. See [HTML viewers](/docs/editing/viewers#html).
